...
namespace Shadowsocks.Controller
{
    internal class APIServer : Listener.Service
    {
        ...
        private bool CheckEnd(string request)
        {
            ...
            {
                if (newline_pos + 4 == request.Length) return true;
            }
            ...
            if (request.IndexOf("POST ") == 0)
            {
                ...
                foreach (var p in get_params)
                    if (p.IndexOf('=') > 0)
                    {
                        ...
                        key = p.Substring(0, index);
                        val = p.Substring(index + 1);
                        params_dict[key] = Utils.urlDecode(val);
                    }
            }

            if (params_dict.ContainsKey("token") && params_dict.ContainsKey("app")
                                                 && _config.token.ContainsKey(params_dict["app"]) &&
                                                 _config.token[params_dict["app"]] == params_dict["token"])
...
", Encoding.UTF8.GetBytes(content).Length) + content;
                    ...
                    if (params_dict["action"] == "config")
                    {
                        if (params_dict.ContainsKey("config"))
                        {
                            var content = "";
                            var ret_code = "200 OK";
                            if (!_controller.SaveServersConfig(params_dict["config"])) ret_code = "403 Forbid";

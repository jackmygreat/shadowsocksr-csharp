...
namespace Shadowsocks.Controller
{
    ...
    public class ProxySocketTun
    {
        ...
        public IAsyncResult BeginReceive(byte[] buffer, int size, SocketFlags flags, AsyncCallback callback,
            object state)
        {
        ...
        public IAsyncResult BeginReceiveFrom(byte[] buffer, int size, SocketFlags flags, ref EndPoint ep,
        ...
        public bool ConnectSocks5ProxyServer(string strRemoteHost, int iRemotePort, bool udp,
            string socks5RemoteUsername, string socks5RemotePassword)
        {
            ...
            if (iRecCount < 2) throw new SocketException(socketErrorCode);
            ...
            if (bySock5Receive[1] != 0) // auth
            {
                ...
                {
                    if (socks5RemoteUsername.Length == 0) throw new SocketException(socketErrorCode);
            ...
            if (!udp) // TCP
            {
                ...
                IPAddress ipAdd;
                var parsed = IPAddress.TryParse(strRemoteHost, out ipAdd);
                if (ipAdd == null)
                {
                    dataSock5Send.Add(3); // remote DNS resolve
                    dataSock5Send.Add((byte) strRemoteHost.Length);
                    for (var i = 0; i < strRemoteHost.Length; ++i) dataSock5Send.Add((byte) strRemoteHost[i]);
                }
                else
                {
                    ...
                    if (addBytes.GetLength(0) > 4)
                    {
                        ...
                        for (var i = 0; i < 16; ++i) dataSock5Send.Add(addBytes[i]);
                    }
                    else
                    {
                        dataSock5Send.Add(1); // IPv4
                        for (var i = 0; i < 4; ++i) dataSock5Send.Add(addBytes[i]);
                    }
                }
        ...
        public bool ConnectHttpProxyServer(string strRemoteHost, int iRemotePort, string socks5RemoteUsername,
            string socks5RemotePassword, string proxyUserAgent)
        {
            _proxy = true;

            IPAddress ipAdd;
            var parsed = IPAddress.TryParse(strRemoteHost, out ipAdd);
            if (ipAdd != null) strRemoteHost = ipAdd.ToString();
            var host = (strRemoteHost.IndexOf(':') >= 0 ? "[" + strRemoteHost + "]" : strRemoteHost) + ":" +
                       iRemotePort;
            var authstr =
                Convert.ToBase64String(Encoding.UTF8.GetBytes(socks5RemoteUsername + ":" + socks5RemotePassword));
            ...
            if (iRecCount > 13)
            {
                ...
                if (data_part.Length > 1 && data_part[1] == "200") return true;
            }
    ...
    internal class ProxyEncryptSocket
    {
        ...
        public void SetObfsPlugin(Server server, int head_len)
        {
            ...
            {
                if (server.getProtocolData() == null) server.setProtocolData(_protocol.InitData());
                if (server.getObfsData() == null) server.setObfsData(_obfs.InitData());
            }
            ...
            _protocol.SetServerInfo(new ServerInfo(server_addr, server.server_port, server.protocolparam ?? "",
                server.getProtocolData(),
                _encryptor.getIV(), _password, _encryptor.getKey(), head_len, mss, OverHead, RecvBufferSize));
            _obfs.SetServerInfo(new ServerInfo(server_addr, server.server_port, server.obfsparam ?? "",
                server.getObfsData(),
                _encryptor.getIV(), _password, _encryptor.getKey(), head_len, mss, OverHead, RecvBufferSize));
        }

        public int Receive(byte[] recv_buffer, int size, SocketFlags flags, out int bytesRead, out int protocolSize,
            out bool sendback)
        {
            ...
            if (bytesRead > 0)
            {
                lock (_decryptionLock)
                {
                    var bytesToSend = 0;
                    int obfsRecvSize;
                    var remoteRecvObfsBuffer =
                        _obfs.ClientDecode(recv_buffer, bytesRead, out obfsRecvSize, out sendback);
                    if (obfsRecvSize > 0)
                    {
                        Utils.SetArrayMinSize(ref ReceiveDecryptBuffer, obfsRecvSize);
                        _encryptor.Decrypt(remoteRecvObfsBuffer, obfsRecvSize, ReceiveDecryptBuffer, out bytesToSend);
                        int outlength;
                        protocolSize = bytesToSend;
                        var buffer = _protocol.ClientPostDecrypt(ReceiveDecryptBuffer, bytesToSend, out outlength);
        ...
        public int EndReceive(IAsyncResult ar, out bool sendback)
        {
            ...
            if (bytesRead > 0)
            {
                ...
                lock (_decryptionLock)
                {
                    var bytesToSend = 0;
                    int obfsRecvSize;
                    var remoteRecvObfsBuffer = _obfs.ClientDecode(st.buffer, bytesRead, out obfsRecvSize, out sendback);
                    if (obfsRecvSize > 0)
                    {
                        Utils.SetArrayMinSize(ref ReceiveDecryptBuffer, obfsRecvSize);
                        _encryptor.Decrypt(remoteRecvObfsBuffer, obfsRecvSize, ReceiveDecryptBuffer, out bytesToSend);
                        ...
                        var buffer = _protocol.ClientPostDecrypt(ReceiveDecryptBuffer, bytesToSend, out outlength);
                        TcpMSS = _protocol.GetTcpMSS();
                        if (st.buffer.Length < outlength) Array.Resize(ref st.buffer, outlength);
        ...
        public int Send(byte[] buffer, int size, SocketFlags flags)
        {
            ...
            lock (_encryptionLock)
            {
                ...
                //    }
                //}
                var bytesToEncrypt = _protocol.ClientPreEncrypt(buffer, size, out outlength);
                if (bytesToEncrypt == null)
                    return 0;
                Utils.SetArrayMinSize(ref SendEncryptBuffer, outlength + 32);
        ...
        public int EndReceiveFrom(IAsyncResult ar, ref EndPoint ep)
        {
            ...
            if (bytesRead > 0)
            {
                ...
                int bytesToSend;
                if (!RemoveRemoteUDPRecvBufferHeader(st.buffer, ref bytesRead)) return 0; // drop
        ...
        public int BeginSendTo(byte[] buffer, int size, SocketFlags flags, AsyncCallback callback, object state)
        {
            ...
            lock (_encryptionLock)
            {
                ...
                int obfsSendSize;
                var obfsBuffer = _protocol.ClientUdpPreEncrypt(bytesToEncrypt, length, out obfsSendSize);
            ...
            if (_proxy)
            {
                ...
                var parsed = IPAddress.TryParse(serverURI, out ipAddress);
                if (!parsed)
                {
                    bytesToEncrypt = new byte[bytes_beg + 1 + 1 + serverURI.Length + 2 + bytesToSend];
                    Array.Copy(connetionSendBuffer, 0, bytesToEncrypt, bytes_beg + 1 + 1 + serverURI.Length + 2,
                        bytesToSend);
                    bytesToEncrypt[0] = 0;
                    bytesToEncrypt[1] = 0;
                    bytesToEncrypt[2] = 0;
                    ...
                    for (var i = 0; i < serverURI.Length; ++i) bytesToEncrypt[5 + i] = (byte) serverURI[i];
                ...
                else
                {
                    ...
                    Array.Copy(connetionSendBuffer, 0, bytesToEncrypt, bytes_beg + 1 + addBytes.Length + 2,
                        bytesToSend);
                    bytesToEncrypt[0] = 0;
                    bytesToEncrypt[1] = 0;
                    bytesToEncrypt[2] = 0;
                    bytesToEncrypt[3] = ipAddress.AddressFamily == AddressFamily.InterNetworkV6 ? (byte) 4 : (byte) 1;
                    for (var i = 0; i < addBytes.Length; ++i) bytesToEncrypt[4 + i] = addBytes[i];

...
namespace Shadowsocks.Controller
{
    ...
    internal class ProxyAuthHandler
    {
        ...
        public ProxyAuthHandler(Configuration config, ServerTransferTotal transfer, IPRangeSet IPRange,
            byte[] firstPacket, int length, Socket socket)
        {
        ...
        {
            if ((_config.authUser ?? "").Length == 0) return true;
            if (_config.authUser == authUser && (_config.authPass ?? "") == authPass) return true;
            if (Utils.isMatchSubNet(((IPEndPoint) _connection.RemoteEndPoint).Address, "127.0.0.0/8")) return true;
            return false;
        }
            ...
            try
            {
                ...
                if (bytesRead > 1)
                {
                    if ((!string.IsNullOrEmpty(_config.authUser) ||
                         Utils.isMatchSubNet(((IPEndPoint) _connection.RemoteEndPoint).Address, "127.0.0.0/8"))
        ...
        private void RspSocks4aHandshakeReceive()
        {
            ...
            for (var i = 0; i < _firstPacketLength; ++i) firstPacket.Add(_firstPacket[i]);
            ...
            if (remoteDNS)
            {
                for (var i = 0; i < 4; ++i) dataSockSend.Add(0);
            ...
            {
                for (var i = 0; i < 4; ++i) dataSockSend.Add(_firstPacket[4 + i]);
        ...
        private void RspSocks5HandshakeReceive()
        {
            ...
            if (auth || !no_auth)
            {
                ...
                HandshakeAuthReceiveCallback();
                                 ...
                                 || Utils.isMatchSubNet(((IPEndPoint) _connection.RemoteEndPoint).Address,
                                     "127.0.0.0/8")))
            {
                _connection.Send(response);
        ...
        private void HandshakeReceive3Callback(int recv_size)
        {
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    Array.Copy(_connetionRecvBuffer, 0, _remoteHeaderSendBuffer,
                        _remoteHeaderSendBuffer.Length - bytesRead, bytesRead);
        ...
        {
            command = 1; // Set TCP connect command
            if (httpProxyState == null) httpProxyState = new HttpPraser();
            if (Utils.isMatchSubNet(((IPEndPoint) _connection.RemoteEndPoint).Address, "127.0.0.0/8"))
            {
            ...
            for (var i = 1;; ++i)
            {
                var err = httpProxyState.HandshakeReceive(_firstPacket, _firstPacketLength,
                    ref _remoteHeaderSendBuffer);
                if (err == 1)
                {
        ...
        private void Connect()
        {
            Handler.GetCurrentServer getCurrentServer =
                delegate(int localPort, ServerSelectStrategy.FilterFunc filter, string targetURI, bool cfgRandom,
                    bool usingRandom, bool forceRandom)
                {
                    return _config.GetCurrentServer(localPort, filter, targetURI, cfgRandom, usingRandom, forceRandom);
                };
            Handler.KeepCurrentServer keepCurrentServer = delegate(int localPort, string targetURI, string id)
            {
            ...
            if (!string.IsNullOrEmpty(_config.localDnsServer)) handler.cfg.local_dns_servers = _config.localDnsServer;
            if (!string.IsNullOrEmpty(_config.dnsServer)) handler.cfg.dns_servers = _config.dnsServer;
            if (_config.GetPortMapCache().ContainsKey(local_port))
            {
                ...
                {
                    if (cfg.server != null)
                        handler.select_server = delegate(Server server, Server selServer)
                        {
                            return server.id == cfg.server.id;
                        };
                    else if (!string.IsNullOrEmpty(cfg.id))
                        handler.select_server = delegate(Server server, Server selServer)
                        {
                            return server.group == cfg.id;
                        };
                    if (cfg.type == PortMapType.Forward) // tunnel
                    {
                        ...
                        handler.Start(_remoteHeaderSendBuffer, _remoteHeaderSendBuffer.Length, null);
                                                    ...
                                                    && new Socks5Forwarder(_config, _IPRange).Handle(
                                                        _remoteHeaderSendBuffer, _remoteHeaderSendBuffer.Length,
                                                        _connection, local_sendback_protocol))
                    {
                    }
                    else
                    {
            ...
            {
                if (_connectionUDP == null && new Socks5Forwarder(_config, _IPRange).Handle(_remoteHeaderSendBuffer,
                    _remoteHeaderSendBuffer.Length, _connection, local_sendback_protocol))
                {
                }
                else
                {

...
namespace Shadowsocks.Controller
{
    ...
    public static class WinINet
    {
        ...
        private static void SetIEProxy(bool enable, bool global, string proxyServer, string pacURL, string connName)
        {
            ...
            else
            {
                ...
                _optionlist.Add(new INTERNET_PER_CONN_OPTION
                {
                    ...
                    Value =
                    {
                        pszValue = Marshal.StringToHGlobalAuto(
                            "localhost;127.*;10.*;172.16.*;172.17.*;172.18.*;172.19.*;172.20.*;172.21.*;172.22.*;172.23.*;172.24.*;172.25.*;172.26.*;172.27.*;172.28.*;172.29.*;172.30.*;172.31.*;172.32.*;192.168.*;<local>")
                    }
                });
            }
            ...
            var len = 0;
            foreach (var option in _optionlist) len += Marshal.SizeOf(option);
            ...
            if (!bReturn) throw new Exception("InternetSetOption failed.", new Win32Exception());
            ...
            bReturn = NativeMethods.InternetSetOption(
                ...
                IntPtr.Zero, 0);
            if (!bReturn) Logging.Error("InternetSetOption:INTERNET_OPTION_PROXY_SETTINGS_CHANGED");

            bReturn = NativeMethods.InternetSetOption(
                IntPtr.Zero,
                (int) INTERNET_OPTION.INTERNET_OPTION_REFRESH,
                IntPtr.Zero, 0);
            if (!bReturn) Logging.Error("InternetSetOption:INTERNET_OPTION_REFRESH");
        }
        ...
        public static void SetIEProxy(bool enable, bool global, string proxyServer, string pacURL)
        {
            ...
            else if (ret == 0)
            {
                ...
                SetIEProxy(enable, global, proxyServer, pacURL, null);
                foreach (var connName in allConnections) SetIEProxy(enable, global, proxyServer, pacURL, connName);
            }
        }
    }
    ...
    internal static class RemoteAccessService
    {
        ...
        public static uint GetAllConns(ref string[] allConns)
        {
            ...
            if (retval == ERROR_BUFFER_TOO_SMALL)
            {
                ...
                for (var i = 0; i < names.Length; i++) names[i].dwSize = entryNameSize;
            ...
            if (retval == ERROR_SUCCESS)
            {
                ...
                for (var i = 0; i < names.Length; i++) allConns[i] = names[i].szEntryName;
                return 0;
            }
    ...
    public class SystemProxy
    {
        ...
        public static void NotifyIE()
        {
            ...
            _settingsReturn = NativeMethods.InternetSetOption(IntPtr.Zero,
                (int) INTERNET_OPTION.INTERNET_OPTION_SETTINGS_CHANGED, IntPtr.Zero, 0);
            _refreshReturn = NativeMethods.InternetSetOption(IntPtr.Zero, (int) INTERNET_OPTION.INTERNET_OPTION_REFRESH,
                IntPtr.Zero, 0);
        }
        ...
        public static void Update(Configuration config, bool forceDisable)
        {
            ...
            if (sysProxyMode == (int) ProxyMode.NoModify) return;
            if (forceDisable) sysProxyMode = (int) ProxyMode.Direct;
            ...
            {
                using (var registry =
                    OpenUserRegKey(@"Software\Microsoft\Windows\CurrentVersion\Internet Settings", true))
                {
                    try
                    {
                        if (enabled)
                        {
                            ...
                            {
                                string pacUrl;
                                pacUrl = "http://127.0.0.1:" + config.localPort + "/pac?" + "auth=" +
                                         config.localAuthPassword + "&t=" + Utils.GetTimestamp(DateTime.Now);
                ...
                try
                {
                    if (enabled)
                    {
                        ...
                        else
                        {
                            string pacUrl;
                            pacUrl =
                                $"http://127.0.0.1:{config.localPort}/pac?auth={config.localAuthPassword}&t={Utils.GetTimestamp(DateTime.Now)}";
        ...
        {
            using (var registry =
                OpenUserRegKey(@"Software\Microsoft\Windows\CurrentVersion\Internet Settings\Connections", true))
            {
                try
                {
        ...
        private static byte[] GenConnectionSettings(Configuration config, int sysProxyMode, int counter)
        {
            ...
            var bypass = sysProxyMode == (int) ProxyMode.Global
                ? ""
                : "localhost;127.*;10.*;172.16.*;172.17.*;172.18.*;172.19.*;172.20.*;172.21.*;172.22.*;172.23.*;172.24.*;172.25.*;172.26.*;172.27.*;172.28.*;172.29.*;172.30.*;172.31.*;172.32.*;192.168.*;<local>";
        ...
        private static void IEProxyUpdate(Configuration config, int sysProxyMode)
        {
            ...
            using (var registry = OpenUserRegKey(@"Software\Microsoft\Windows\CurrentVersion\Internet Settings", true))
            {
                try
                {
                    RegistrySetValue(registry, "ProxyOverride",
                        "localhost;127.*;10.*;172.16.*;172.17.*;172.18.*;172.19.*;172.20.*;172.21.*;172.22.*;172.23.*;172.24.*;172.25.*;172.26.*;172.27.*;172.28.*;172.29.*;172.30.*;172.31.*;172.32.*;192.168.*;<local>");
                }
                catch (IOException e)
                {

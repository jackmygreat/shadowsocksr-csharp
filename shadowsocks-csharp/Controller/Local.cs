...
namespace Shadowsocks.Controller
{
    ...
    internal class Local : Listener.Service
    {
        ...
        {
            if (!_config.GetPortMapCache().ContainsKey(((IPEndPoint) socket.LocalEndPoint).Port) &&
                !Accept(firstPacket, length)) return false;
        ...
        protected bool Accept(byte[] firstPacket, int length)
        {
            ...
            if (firstPacket[0] == 5 || firstPacket[0] == 4) return true;
            if (length > 8
                && firstPacket[0] == 'C'
                && firstPacket[1] == 'O'
                && firstPacket[2] == 'N'
    ...
    internal class Handler
        : IHandler
    {
        public delegate Server GetCurrentServer(int localPort, ServerSelectStrategy.FilterFunc filter,
            string targetURI = null, bool cfgRandom = false, bool usingRandom = false, bool forceRandom = false);
        ...
        private void timer_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            try
            {
                if (cfg.try_keep_alive <= 0 && State == ConnectState.CONNECTED && remote != null && remoteUDP == null &&
                    remote.CanSendKeepAlive)
                {
                    cfg.try_keep_alive++;
                ...
                else if (se.SocketErrorCode == SocketError.HostNotFound)
                {
                    if (lastErrCode == 0)
                        if (s != null)
                        {
                            lastErrCode = 2;
                            s.ServerSpeedLog().AddErrorTimes();
                            if (s.ServerSpeedLog().ErrorConnectTimes >= 3 && cfg.autoSwitchOff) s.setEnable(false);
                        }
        ...
        public bool ReConnect()
        {
            Logging.Debug("Reconnect " + cfg.targetHost + ":" + cfg.targetPort + " " + connection.GetSocket().Handle);
            {
                ...
                {
                    foreach (var data in connectionSendBufferList) total_len += data.Length;
        ...
        public void Start(byte[] firstPacket, int length, string rsp_protocol)
        {
            ...
            if (cfg.socks5RemotePort > 0) cfg.autoSwitchOff = false;
        ...
        private void BeginConnect(IPAddress ipAddress, int serverPort)
        {
            ...
            if (connectionUDP != null && !server.udp_over_tcp)
                try
                {
                    ...
                    remoteUDP.GetSocket()
                        .Bind(new IPEndPoint(
                            ipAddress.AddressFamily == AddressFamily.InterNetworkV6 ? IPAddress.IPv6Any : IPAddress.Any,
                            0));
            ...
            {
                var _state = State;
                if (_state == ConnectState.CONNECTING) StartPipe();
            }
            else
            {
        ...
        private bool ConnectProxyServer(string strRemoteHost, int iRemotePort)
        {
            if (cfg.proxyType == 0)
            {
                var ret = remote.ConnectSocks5ProxyServer(strRemoteHost, iRemotePort,
                    connectionUDP != null && !server.udp_over_tcp, cfg.socks5RemoteUsername, cfg.socks5RemotePassword);
                remote.SetTcpServer(server.server, server.server_port);
                remote.SetUdpServer(server.server,
                    server.server_udp_port == 0 ? server.server_port : server.server_udp_port);
                if (remoteUDP != null)
                {
                    remoteUDP.GoS5Proxy = true;
                    remoteUDP.SetUdpServer(server.server,
                        server.server_udp_port == 0 ? server.server_port : server.server_udp_port);
            ...
            if (cfg.proxyType == 1)
            {
                var ret = remote.ConnectHttpProxyServer(strRemoteHost, iRemotePort, cfg.socks5RemoteUsername,
                    cfg.socks5RemotePassword, cfg.proxyUserAgent);
                remote.SetTcpServer(server.server, server.server_port);
                return ret;
            }
        ...
        private void Connect()
        {
            ...
            Logging.Info(
                $"Connect {cfg.targetHost}:{cfg.targetPort.ToString()} via {server.server}:{server.server_port}");
            ...
            lock (this)
            {
                ...
                if (State == ConnectState.HANDSHAKE) State = ConnectState.CONNECTING;
            ...
            try
            {
                ...
                var parsed = IPAddress.TryParse(serverHost, out ipAddress);
                if (!parsed)
                {
        ...
        private void doConnectionUDPRecv()
        {
            if (connectionUDP != null && connectionUDPIdle)
            {
                ...
                var sender =
                    new IPEndPoint(
                        connectionUDP.AddressFamily == AddressFamily.InterNetworkV6 ? IPAddress.IPv6Any : IPAddress.Any,
                        0);
        ...
        private int endRemoteTCPRecv(IAsyncResult ar)
        {
            if (remote != null)
            {
                ...
                server.ServerSpeedLog()
                    .AddDownloadBytes(bytesRecv, DateTime.Now, speedTester.AddDownloadSize(bytesRecv));
        ...
        private void doRemoteUDPRecv()
        {
            if (remoteUDP != null && remoteUDPIdle)
            {
                ...
                var sender =
                    new IPEndPoint(
                        remoteUDP.AddressFamily == AddressFamily.InterNetworkV6 ? IPAddress.IPv6Any : IPAddress.Any, 0);
                var tempEP = (EndPoint) sender;
        ...
        private string GetQueryString()
        {
            ...
            if (remoteHeaderSendBuffer[0] == 3 && remoteHeaderSendBuffer.Length > 1)
                ...
                {
                    var url = Encoding.UTF8.GetString(remoteHeaderSendBuffer, 2, remoteHeaderSendBuffer[1]);
                    return url;
                }
        ...
        private int GetQueryPort()
        {
            ...
            if (remoteHeaderSendBuffer[0] == 3 && remoteHeaderSendBuffer.Length > 1)
                ...
                {
                    var port = (remoteHeaderSendBuffer[remoteHeaderSendBuffer[1] + 2] << 8) |
                               remoteHeaderSendBuffer[remoteHeaderSendBuffer[1] + 3];
                    return port;
                }
        ...
        private void StartPipe()
        {
            try
            {
                ...
                if (connection.local_sendback_protocol != null) connection.Send(remoteUDPRecvBuffer, 0, 0);
        ...
        private void ConnectionSend(byte[] buffer, int bytesToSend)
        {
            ...
            {
                connectionUDP.BeginSendTo(buffer, 0, bytesToSend, SocketFlags.None, connectionUDPEndPoint,
                    PipeConnectionUDPSendCallback, null);
            }
        }
            ...
            if (buffer_list.Count == 0)
                ...
                foreach (var buffer in buffer_list)
                    if (buffer == buffer_list[buffer_list.Count - 1])
                        connectionUDP.BeginSendTo(buffer, 0, buffer.Length, SocketFlags.None, connectionUDPEndPoint,
                            PipeConnectionUDPSendCallback, null);
                    else
                        connectionUDP.BeginSendTo(buffer, 0, buffer.Length, SocketFlags.None, connectionUDPEndPoint,
                            PipeConnectionUDPSendCallbackNoRecv, null);
        }
                ...
                else
                {
                    ...
                    if (speedTester.BeginDownload())
                    {
                        ...
                        if (speedTester.timeBeginDownload != null && speedTester.timeBeginUpload != null)
                            pingTime = (int) (speedTester.timeBeginDownload - speedTester.timeBeginUpload)
                                .TotalMilliseconds;
                        if (pingTime >= 0)
        ...
        private void PipeRemoteReceiveLoop()
        {
            ...
            while (!closed)
                try
                {
                    ...
                    bool sendback;
                    int bytesRecv;
                    var bytesRead = remote.Receive(recv_buffer, RecvSize, 0, out bytesRecv, out protocolSize,
                        out sendback);
        ...
        private int RemoteSend(byte[] bytes, int length)
        {
            ...
            if (send_len > 0)
            {
                ...
                {
                    if (keepCurrentServer != null) keepCurrentServer(localPort, cfg.targetHost, server.id);
                ...
                while (true)
                {
                    ...
                    {
                        server.ServerSpeedLog()
                            .AddUploadBytes(send_len, DateTime.Now, speedTester.AddUploadSize(send_len));
                        total_len += send_len;
                    }
                    else
                    {
            ...
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    if (State == ConnectState.CONNECTED)
                    {
                        if (remoteHeaderSendBuffer != null)
                        {
                            Array.Copy(connetionRecvBuffer, 0, connetionRecvBuffer, remoteHeaderSendBuffer.Length,
                                bytesRead);
                            Array.Copy(remoteHeaderSendBuffer, 0, connetionRecvBuffer, 0,
                                remoteHeaderSendBuffer.Length);
                    ...
                    if (closed || State != ConnectState.CONNECTED) return;
                    if (connectionSendBufferList != null)
                        ResetTimeout(cfg.connect_timeout);
                    else
                        ResetTimeout(cfg.TTL);

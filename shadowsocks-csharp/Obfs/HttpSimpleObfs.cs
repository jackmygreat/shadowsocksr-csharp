...
namespace Shadowsocks.Obfs
{
    internal class HttpSimpleObfs : ObfsBase
    {
        ...
        private static readonly string[] _request_path =
        {
            "", "",
            "login.php?redir=", "",
            "register.php?code=", "",
        ...
        private static readonly string[] _request_useragent =
        {
        ...
        private readonly List<byte[]> data_buffer = new List<byte[]>();
        ...
        {
            var ret = "";
            for (var i = 0; i < datalength; ++i) ret += "%" + encryptdata[i].ToString("x2");
            return ret;
        }
        ...
        private string boundary()
        {
            ...
            var ret = "";
            for (var i = 0; i < 32; ++i) ret += set[random.Next(set.Length)];
            return ret;
        }
        ...
        public override byte[] ClientEncode(byte[] encryptdata, int datalength, out int outlength)
        {
            ...
            else if (Method == "http_simple" || Method == "http_post")
            {
                ...
                var http_buf =
                    (Method == "http_post" ? "POST /" : "GET /") + _request_path[request_path_index] +
                    data2urlencode(headdata, headdata.Length) + _request_path[request_path_index + 1] + " HTTP/1.1\r\n"
                ...
                if (custom_head.Length > 0)
                    ...
                    http_buf +=
                        ...
                        + "\r\n";
                for (var i = 0; i < http_buf.Length; ++i) outdata[i] = (byte) http_buf[i];
            ...
            for (var pos = 0; pos < length; ++pos)
            {
                ...
                for (; offset < subArray.Length; ++offset)
                    if (array[pos + offset] != subArray[offset])
                        break;
                if (offset == subArray.Length) return pos;
            }
    ...
    internal class TlsTicketAuthObfs : ObfsBase
    {
        ...
        private readonly List<byte[]> data_sent_buffer = new List<byte[]>();
        ...
        {
            if (url == null) url = "";
            var b_url = Encoding.UTF8.GetBytes(url);
        ...
        protected byte[] to_bin(string str)
        {
            ...
            for (var i = 0; i < str.Length; i += 2) ret[i / 2] = (byte) ((to_val(str[i]) << 4) | to_val(str[i + 1]));
            return ret;
        }
        ...
        public void PackAuthData(byte[] outdata)
        {
            ...
            var utc_time_second =
                (ulong) Math.Floor(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1, 0, 0, 0)).TotalSeconds);
        ...
        public override byte[] ClientEncode(byte[] encryptdata, int datalength, out int outlength)
        {
            ...
            if ((handshake_status & 3) != 0)
            {
                ...
                if ((handshake_status & 2) == 0)
                {
                    ...
                    var handshake_finish = Encoding.ASCII.GetBytes("\x14\x03\x03\x00\x01\x01" + "\x16\x03\x03\x00\x20");
                ...
                if (datalength == 0 || fastauth)
                {
                    ...
                    {
                        Utils.SetArrayMinSize2(ref outdata, outlength + data.Length);
                        Array.Copy(data, 0, outdata, outlength, data.Length);
                        SentLength += data.Length;
                        outlength += data.Length;
                    }
                ...
                if (datalength == 0) handshake_status |= 4;
            }
            else
            {
                var rnd = new byte[32];
                ...
                if (!string.IsNullOrEmpty(host) && host[host.Length - 1] >= '0' && host[host.Length - 1] <= '9' &&
                    Server.param.Length == 0) host = "";
                ...
                ext_buf.AddRange(to_bin(str_buf2));
                {
                    ...
                    lock (authData)
                    {
                        if (authData.ticket_buf == null) authData.ticket_buf = new Dictionary<string, byte[]>();
                ...
                var str_buf3 =
                    "000d0016001406010603050105030401040303010303020102030005000501000000000012000075500000000b00020100000a0006000400170018";
                str_buf3 +=
                ...
                for (var i = 0; i < ssl_buf.Count; ++i) outdata[i] = ssl_buf[i];
            ...
            out bool needsendback)
        {
            ...
            else
            {
                ...
                if (data_recv_buffer.Length >= 11 + 32 + 1 + 32)
                {
                    ...
                    if (!Utils.BitCompare(data_recv_buffer, 11 + 22, data, 22, 10))
                    ...
                    if (!Utils.BitCompare(data_recv_buffer, headerlength - 10, data, headerlength - 10, 10))
                    {
                        headerlength = 0;
                        ...
                        {
                            headerlength += 5;
                            if (headerlength >= data_recv_buffer.Length) return encryptdata;
                            headerlength += (data_recv_buffer[headerlength - 2] << 8) |
                                            data_recv_buffer[headerlength - 1];
                            if (headerlength > data_recv_buffer.Length) return encryptdata;
                        }

...
namespace Shadowsocks.Obfs
{
    ...
    public class AuthSHA1 : VerifySimpleBase
    {
        ...
        public void PackData(byte[] data, int datalength, byte[] outdata, out int outlength)
        {
            ...
            var adler = Adler32.CalcAdler32(outdata, outlength - 4);
        ...
        public void PackAuthData(byte[] data, int datalength, byte[] outdata, out int outlength)
        {
            ...
            lock (authData)
            {
                if (authData.connectionID > 0xFF000000) authData.clientID = null;
            ...
            var crc32 = CRC32.CalcCRC32(Server.key, Server.key.Length);
        ...
        public override byte[] ClientPostDecrypt(byte[] plaindata, int datalength, out int outlength)
        {
            ...
            while (recv_buf_len > 2)
            {
                ...
                if (len >= 8192 || len < 7) throw new ObfsException("ClientPostDecrypt data error");
                if (len > recv_buf_len)
                    break;

                if (Adler32.CheckAdler32(recv_buf, len))
                {
    ...
    public class AuthSHA1V2 : VerifySimpleBase
    {
        ...
        public void PackData(byte[] data, int datalength, byte[] outdata, out int outlength)
        {
            var rand_len = (datalength >= 1300 ? 0 : datalength > 400 ? LinearRandomInt(128) : LinearRandomInt(1024)) +
                           1;
            outlength = rand_len + datalength + 6;
            if (datalength > 0)
                Array.Copy(data, 0, outdata, rand_len + 2, datalength);
        ...
        public override byte[] ClientPreEncrypt(byte[] plaindata, int datalength, out int outlength)
        {
            ...
            while (datalength > unit_len)
            {
                int outlen;
                PackData(data, unit_len, packdata, out outlen);
                Utils.SetArrayMinSize(ref outdata, outlength + outlen);
                Array.Copy(packdata, 0, outdata, outlength, outlen);
                outlength += outlen;
                datalength -= unit_len;
                var newdata = new byte[datalength];
        ...
        public override byte[] ClientPostDecrypt(byte[] plaindata, int datalength, out int outlength)
        {
            ...
            {
                var len = (recv_buf[0] << 8) + recv_buf[1];
                if (len >= 8192 || len < 8) throw new ObfsException("ClientPostDecrypt data error");
                if (len > recv_buf_len)
                    break;
    ...
    public class AuthSHA1V4 : VerifySimpleBase
    {
        ...
        public void PackData(byte[] data, int datalength, byte[] outdata, out int outlength)
        {
            ...
            outdata[0] = (byte) (outlength >> 8);
            outdata[1] = (byte) outlength;
            var crc32 = CRC32.CalcCRC32(outdata, 2);
        ...
        public override byte[] ClientPostDecrypt(byte[] plaindata, int datalength, out int outlength)
        {
            ...
            {
                var crc32 = CRC32.CalcCRC32(recv_buf, 2);
    ...
    public class AuthAES128SHA1 : VerifySimpleBase
    {
        ...
        protected void StatisticsInit(AuthDataAes128 authData)
        {
            ...
            {
                authData.tree = new MinSearchTree(Server.tcp_mss - tree_offset);
                authData.tree.Init();
            }
        ...
        public void PackData(byte[] data, int datalength, int fulldatalength, byte[] outdata, out int outlength,
...
#if !PROTOCOL_STATISTICS
            int rand_len =
 TrapezoidRandomInt(Server.tcp_mss - datalength - overhead + 1, -0.3); //(datalength > 400 ? LinearRandomInt(512) : LinearRandomInt(1024));
#else
...

            {
                ...
                if (index_of_split > 0)
                    try
                    {
                        ...
                        user_key = hash(Encoding.UTF8.GetBytes(Server.param.Substring(index_of_split + 1)));
                ...
                var encryptor =
                    EncryptorFactory.GetEncryptor("aes-128-cbc", Convert.ToBase64String(encrypt_key) + SALT, false);
                int enc_outlen;
        ...
        public override byte[] ClientPreEncrypt(byte[] plaindata, int datalength, out int outlength)
        {
            ...
            const int unit_len = 8100;
            var ogn_datalength = datalength;
            if (datalength < 0 || last != null && (now - last).TotalSeconds > 3) Sync();
            if (!has_sent_header)
            {
            ...
            {
                {
                    ...
                    {
                        send_len =
 TrapezoidRandomInt(Math.Min(datalength - 1, Server.tcp_mss - overhead) - 1, -0.3) + 1;  // must less than datalength
#else
            if (datalength > 120 * 4 && pack_id < 64)
            {
                ...
                {
                    var max_packet_size =
                        Math.Min(datalength - 1 + overhead, Server.tcp_mss); // must less than datalength + overhead

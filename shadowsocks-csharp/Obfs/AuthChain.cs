...
namespace Shadowsocks.Obfs
{
    internal class xorshift128plus
    {
        ...
        public void init_from_bin(byte[] bytes, int datalength)
        {
            ...
            v0 = BitConverter.ToUInt64(fill_bytes, 0);
            v1 = BitConverter.ToUInt64(fill_bytes, 8);
            for (var i = 0; i < init_loop; ++i) next();
        }
    }
    ...
    internal class AuthChain_a : VerifySimpleBase
    {
        ...
        public void PackAuthData(byte[] data, int datalength, byte[] outdata, out int outlength)
        {
            ...
            // uid & 16 bytes auth data
            {
                ...
                if (index_of_split > 0)
                    try
                    {
                        ...
                        user_key = Encoding.UTF8.GetBytes(Server.param.Substring(index_of_split + 1));
                ...
                for (var i = 0; i < 4; ++i) uid[i] ^= last_client_hash[8 + i];
            ...
            encryptor = EncryptorFactory.GetEncryptor("rc4",
                Convert.ToBase64String(user_key) + Convert.ToBase64String(last_client_hash, 0, 16), false);
        ...
        public override byte[] ClientPreEncrypt(byte[] plaindata, int datalength, out int outlength)
        {
            ...
            if (datalength > 120 * 4 && pack_id < 32)
            {
                ...
                {
                    send_len = TrapezoidRandomInt(Math.Min(datalength - 1, Server.tcp_mss - overhead) - 1, -0.3) +
                               1; // must less than datalength
        ...
        public override byte[] ClientPostDecrypt(byte[] plaindata, int datalength, out int outlength)
        {
            ...
            while (recv_buf_len > 4)
            {
                ...
                if (len >= 4096) throw new ObfsException("ClientPostDecrypt data error");
        ...
        public override byte[] ClientUdpPreEncrypt(byte[] plaindata, int datalength, out int outlength)
        {
            ...
            encryptor = EncryptorFactory.GetEncryptor("rc4",
                Convert.ToBase64String(user_key) + Convert.ToBase64String(md5data, 0, 16), false);
            ...
            var uid = new byte[4];
            for (var i = 0; i < 4; ++i) uid[i] = (byte) (user_id[i] ^ md5data[i]);
    ...
    internal class AuthChain_b : AuthChain_a
    {
        ...
        public new static List<string> SupportedObfs()
        {
            return new List<string>(_obfs.Keys);
        }
        ...
        protected virtual void InitDataSizeList()
        {
            ...
            for (var i = 0; i < len; ++i) data_list.Add((int) (random.next() % 2340 % 2040 % 1440));
            data_list.Sort();
        ...
        protected int FindPos(int[] arr, int key)
        {
            ...
            while (low < high)
            {
                ...
                if (key > arr[middle])
                    low = middle + 1;
                else if (key <= arr[middle]) high = middle;
            }
        ...
        protected override int GetRandLen(int datalength, xorshift128plus random, byte[] last_hash)
        {
            ...
            if (final_pos < data_size_list.Length) return data_size_list[final_pos] - datalength - Server.overhead;

            pos = FindPos(data_size_list2, datalength + Server.overhead);
            final_pos = pos + (int) (random.next() % (ulong) data_size_list2.Length);
            if (final_pos < data_size_list2.Length) return data_size_list2[final_pos] - datalength - Server.overhead;
            if (final_pos < pos + data_size_list2.Length - 1) return 0;
            if (datalength > 1300)
                return (int) (random.next() % 31);
            if (datalength > 900)
                return (int) (random.next() % 127);
    ...
    internal class AuthChain_d : AuthChain_c
    {
        ...
        protected override void InitDataSizeList()
        {
            ...
            CheckAndPatchDataSize(data_list, random);
            if (old_len != data_list.Count) data_list.Sort();
            data_size_list0 = data_list.ToArray();
        }
        ...
        protected override int GetRandLen(int datalength, xorshift128plus random, byte[] last_hash)
        {
            ...
            if (other_data_size >= data_size_list0[data_size_list0.Length - 1]) return 0;
    ...
    internal class AuthChain_f : AuthChain_e
    {
        ...
        protected override void InitDataSizeList()
        {
            ...
            for (var i = 0; i < 8; i++) newKey[i] ^= key_change_datetime_key_bytes[i];
        ...
        public override void SetServerInfo(ServerInfo serverInfo)
        {
            ...
            if (protocalParams != "")
            {
                ...
                ulong interval;
                if (ulong.TryParse(protocalParams, out interval)) key_change_interval = interval;
            }
        }

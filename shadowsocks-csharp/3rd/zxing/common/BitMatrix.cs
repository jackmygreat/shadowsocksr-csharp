   ...
   public sealed class BitMatrix
    {
        ...
        {
            if (width < 1 || height < 1) throw new ArgumentException("Both dimensions must be greater than 0");
            Width = width;
            Height = height;
            rowSize = (width + 31) >> 5;
        ...
        {
            if (top < 0 || left < 0) throw new ArgumentException("Left and top must be nonnegative");
            if (height < 1 || width < 1) throw new ArgumentException("Height and width must be at least 1");
            var right = left + width;
            var bottom = top + height;
            if (bottom > Height || right > Width) throw new ArgumentException("The region must fit inside the matrix");
            for (var y = top; y < bottom; y++)
            {
                var offset = y * rowSize;
                for (var x = left; x < right; x++) bits[offset + (x >> 5)] |= 1 << (x & 0x1f);
            }
        }
        ...
        public BitArray getRow(int y, BitArray row)
        {
            if (row == null || row.Size < Width)
                ...
                row.clear();
            var offset = y * rowSize;
            for (var x = 0; x < rowSize; x++) row.setBulk(x << 5, bits[offset + x]);
            return row;
        }

   ...
   internal sealed class GenericGFPoly
    {
        ...
        {
            if (coefficients.Length == 0) throw new ArgumentException();
            this.field = field;
        ...
        internal int evaluateAt(int a)
        {
            ...
            if (a == 1)
            {
                ...
                foreach (var coefficient in Coefficients) result = GenericGF.addOrSubtract(result, coefficient);
                return result;
            }

            result = Coefficients[0];
            for (var i = 1; i < size; i++) result = GenericGF.addOrSubtract(field.multiply(a, result), Coefficients[i]);
            return result;
        }
        ...
        internal GenericGFPoly[] divide(GenericGFPoly other)
        {
            if (!field.Equals(other.field))
                throw new ArgumentException("GenericGFPolys do not have same GenericGF field");
            if (other.isZero) throw new ArgumentException("Divide by 0");

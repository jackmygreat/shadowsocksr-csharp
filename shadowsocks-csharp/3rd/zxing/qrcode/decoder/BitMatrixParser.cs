...
namespace ZXing.QrCode.Internal
{
    ...
    internal sealed class BitMatrixParser
    {
        ...
        internal static BitMatrixParser createBitMatrixParser(BitMatrix bitMatrix)
        {
            ...
            if (dimension < 21 || (dimension & 0x03) != 1) return null;
        ...
        internal FormatInformation readFormatInformation()
        {
            ...
            for (var i = 0; i < 6; i++) formatInfoBits1 = copyBit(i, 8, formatInfoBits1);
            // .. and skip a bit in the timing pattern ...
            formatInfoBits1 = copyBit(7, 8, formatInfoBits1);
            formatInfoBits1 = copyBit(8, 8, formatInfoBits1);
            formatInfoBits1 = copyBit(8, 7, formatInfoBits1);
            // .. and skip a bit in the timing pattern ...
            for (var j = 5; j >= 0; j--) formatInfoBits1 = copyBit(8, j, formatInfoBits1);
            // Read the top-right/bottom-left pattern too
            var dimension = bitMatrix.Height;
            var formatInfoBits2 = 0;
            var jMin = dimension - 7;
            for (var j = dimension - 1; j >= jMin; j--) formatInfoBits2 = copyBit(8, j, formatInfoBits2);
            for (var i = dimension - 8; i < dimension; i++) formatInfoBits2 = copyBit(i, 8, formatInfoBits2);
        ...
        internal Version readVersion()
        {
            ...
            if (parsedVersion != null && parsedVersion.DimensionForVersion == dimension) return parsedVersion;
        ...
        internal byte[] readCodewords()
        {
                ...
                for (var count = 0; count < dimension; count++)
                {
                    ...
                    for (var col = 0; col < 2; col++)
                        ...
                        if (!functionPattern[j - col, i])
                        {
                            ...
                            bitsRead++;
                            currentByte <<= 1;
                            if (bitMatrix[j - col, i]) currentByte |= 1;
            ...
            if (resultOffset != version.TotalCodewords) return null;
            return result;
        }
       ...
       */
        ...
        {
            if (parsedFormatInfo == null) return; // We have no format information, and have no data mask

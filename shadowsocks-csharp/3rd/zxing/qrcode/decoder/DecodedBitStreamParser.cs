   ...
   internal static class DecodedBitStreamParser
    {
            ...
            try
            {
                ...
                do
                {
                    ...
                    if (mode != Mode.TERMINATOR)
                    {
                        ...
                        {
                            if (bits.available() < 16) return null;
            ...
            int count)
        {
            ...
            if (count * 13 > bits.available()) return false;
        ...
        {
            // Don't crash trying to read more bits than we have available.
            if (count << 3 > bits.available()) return false;

            var readBytes = new byte[count];
            for (var i = 0; i < count; i++) readBytes[i] = (byte) bits.readBits(8);
            string encoding;
            ...
            int count)
        {
            ...
            while (count >= 3)
            {
                ...
                if (threeDigitsBits >= 1000) return false;
            ...
            if (count == 2)
            {
                ...
                if (twoDigitsBits >= 100) return false;

...
namespace ZXing.QrCode.Internal
{
    ...
    public static class Encoder
    {
        ...
        internal static void terminateBits(int numDataBytes, BitArray bits)
        {
            ...
            if (bits.Size > capacity)
                throw new WriterException("data bits cannot fit in the QR Code" + bits.Size + " > " +
                                          capacity);
            for (var i = 0; i < 4 && bits.Size < capacity; ++i) bits.appendBit(false);
            ...
            for (var i = 0; i < numPaddingBytes; ++i) bits.appendBits((i & 0x01) == 0 ? 0xEC : 0x11, 8);
            if (bits.Size != capacity) throw new WriterException("Bits size does not equal capacity");
        }
        ...
        internal static byte[] generateECBytes(byte[] dataBytes, int numEcBytesInBlock)
        {
            ...
            for (var i = 0; i < numDataBytes; i++) toEncode[i] = dataBytes[i] & 0xFF;
            new ReedSolomonEncoder(GenericGF.QR_CODE_FIELD_256).encode(toEncode, numEcBytesInBlock);

            var ecBytes = new byte[numEcBytesInBlock];
            for (var i = 0; i < numEcBytesInBlock; i++) ecBytes[i] = (byte) toEncode[numDataBytes + i];
            return ecBytes;
        }
        ...
        internal static void append8BitBytes(string content, BitArray bits, string encoding)
        {
         ...
         catch (PlatformNotSupportedException)
         {
            ...
            foreach (var b in bytes) bits.appendBits(b, 8);
        }
    }
}
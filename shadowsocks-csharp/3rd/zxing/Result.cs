   ...
   public sealed class Result
    {
       ...
       public Result(string text, byte[] rawBytes, ResultPoint[] resultPoints, BarcodeFormat format, long timestamp)
        {
            if (text == null && rawBytes == null) throw new ArgumentException("Text and bytes are null");
            Text = text;
            RawBytes = rawBytes;
        ...
        {
            if (ResultMetadata == null) ResultMetadata = new Dictionary<ResultMetadataType, object>();
        ...
        {
            if (Text == null) return "[" + RawBytes.Length + " bytes]";
            return Text;
        }
    }
}
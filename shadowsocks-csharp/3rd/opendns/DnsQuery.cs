...
namespace OpenDNS
{
    ...
    public class DnsQuery
    {
        ...
        //Public Properties
        public DnsResponse Response { get; private set; }
        ...
        private byte[] MakeQuery()
        {
            ...
            for (var j = 0; j < tokens.Length; j++)
            {
                ...
                for (var k = 0; k < b.Length; k++) Question[Cursor++] = b[k];
            }
        ...
        private void GetResourceRecord(int i, ResourceRecordCollection Container)
        {
            ...
            var TTL_Seconds = ((data[position++] & byte.MaxValue) << 24) | ((data[position++] & byte.MaxValue) << 16) |
                              ((data[position++] & byte.MaxValue) << 8) | (data[position++] & byte.MaxValue);
            ...
            switch (ResourceType)
            {
                    ...
                    //Get IP Address Blocks
                {
                    byte[] bs =
                    {
                        (byte) (data[position++] & byte.MaxValue), (byte) (data[position++] & byte.MaxValue),
                        (byte) (data[position++] & byte.MaxValue), (byte) (data[position++] & byte.MaxValue)
                    };
                ...
                case Types.AAAA:
                {
                    ...
                    var bs = new ushort[8];
                    for (var j = 0; j < 8; ++j)
                        bs[j] = (ushort) (((byte) (data[position + j * 2] & byte.MaxValue) << 8) |
                                          (byte) (data[position + j * 2 + 1] & byte.MaxValue));
                    position += 16;
                ...
                case Types.SOA:
                {
                    ...
                    //32 bit fields
                    long Serial = ((data[position++] & byte.MaxValue) << 24) |
                                  ((data[position++] & byte.MaxValue) << 16) |
                                  ((data[position++] & byte.MaxValue) << 8) | (data[position++] & byte.MaxValue);
                    long Refresh = ((data[position++] & byte.MaxValue) << 24) |
                                   ((data[position++] & byte.MaxValue) << 16) |
                                   ((data[position++] & byte.MaxValue) << 8) | (data[position++] & byte.MaxValue);
                    long Retry = ((data[position++] & byte.MaxValue) << 24) |
                                 ((data[position++] & byte.MaxValue) << 16) |
                                 ((data[position++] & byte.MaxValue) << 8) | (data[position++] & byte.MaxValue);
                    long Expire = ((data[position++] & byte.MaxValue) << 24) |
                                  ((data[position++] & byte.MaxValue) << 16) |
                                  ((data[position++] & byte.MaxValue) << 8) | (data[position++] & byte.MaxValue);
                    long Minimum = ((data[position++] & byte.MaxValue) << 24) |
                                   ((data[position++] & byte.MaxValue) << 16) |
                                   ((data[position++] & byte.MaxValue) << 8) | (data[position++] & byte.MaxValue);

                    var rrSOA = new SOA(ResourceName, ResourceType, ResourceClass, TTL_Seconds, Server, Email, Serial,
                ...
                case Types.TXT:
                    ...
                    var rrTXT = new ResourceRecord(ResourceName, ResourceType, ResourceClass, TTL_Seconds,
                        ResourceDataText);
                    Container.Add(rrTXT);
                    break;
        ...
        private int ExtractName(int ResourceDataCursor, StringBuilder Name)
        {
            ...
            if (LengthLabel == 0) return ResourceDataCursor;

            do
            {
                if ((LengthLabel & 0xC0) == 0xC0)
                {
                    if (ResourceDataCursor >= length) return -1;

                    //Compression OffsetID for RDATA Compression
                    var CompressionOffsetID =
                        ((LengthLabel & 0x3F) << 8) | (data[ResourceDataCursor++] & byte.MaxValue);
                    ExtractName(CompressionOffsetID, Name);
                    return ResourceDataCursor;
                }

                if (ResourceDataCursor + LengthLabel > length) return -1;

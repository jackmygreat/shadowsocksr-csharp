...
namespace Shadowsocks.Model
{
    ...
    public class MinSearchTree
    {
        ...
        public void Init()
        {
            ...
            for (var i = _count; i < 2 << _level; ++i) _tree[i].min = long.MaxValue;
            var offset = 1 << _level;
            for (var i = _count >> 1; i < offset; ++i) Maintain(i);
        }

        public MinSearchTree Clone()
        {
            var tree = new MinSearchTree(_size);
            for (var i = 0; i < 2 << _level; ++i) tree._tree[i] = _tree[i];
            return tree;
        }
        ...
        public int FindMinCount(int index, int range_min, int range_max, out long min_val)
        {
            ...
            {
                long out_val;
                var cnt = FindMinCount(l, range_min, Math.Min(range_max, _tree[l].range_max), out out_val);
            ...
            {
                long out_val;
                var cnt = FindMinCount(r, Math.Max(range_min, _tree[r].range_min), range_max, out out_val);
                if (out_val < sub_min_val)
                {
                    sub_min_val = out_val;
                    count = cnt;
        ...
        {
            if (_tree[index].range_min + 1 == _tree[index].range_max) return index - (1 << _level);
            var l = index * 2;
            var r = l + 1;
            if (_tree[l].range_max > range_min)
            {
                ...
                {
                    long out_val;
                    var cnt = FindMinCount(l, range_min, _tree[l].range_max, out out_val);
        ...
        {
            long out_val;
            var count = FindMinCount(1, range_min, range_max, out out_val);
        ...
        {
            long ret;
            var cnt = FindMinCount(1, range_min, range_max, out ret);
            return ret;
        }
    }
}
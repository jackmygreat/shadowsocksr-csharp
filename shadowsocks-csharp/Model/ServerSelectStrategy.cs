...
namespace Shadowsocks.Model
{
    public class ServerSelectStrategy
    {
        ...
        {
            if (serverSpeedLog.ErrorContinurousTimes >= 20) return 1;

            if (serverSpeedLog.ErrorContinurousTimes >= 10) return MIN_CHANCE;

            if (serverSpeedLog.AvgConnectTime < 0 && serverSpeedLog.TotalConnectTimes >= 3) return MIN_CHANCE;

            if (serverSpeedLog.TotalConnectTimes < 1) return MAX_CHANCE;
        ...
        private double
            Algorithm4(ServerSpeedLog serverSpeedLog, long avg_speed, double zero_chance) // perfer fast speed
        {
            if (serverSpeedLog.ErrorContinurousTimes >= 20) return 1;
        ...
        protected int SubSelect(List<Server> configs, int curIndex, string algorithm, FilterFunc filter,
            bool forceChange)
        {
            ...
            {
                if (configs.Count > curIndex && curIndex >= 0 && algorithm != "Timer") lastSelectIndex = curIndex;
                ...
                if (serverList.Count > 0)
                {
                             ...
                             || algorithm == "FastDownloadSpeed")
                    {
                        if (algorithm == "Timer")
                        {
                            ...
                            {
                                if (configs.Count > lastSelectIndex && lastSelectIndex >= 0 &&
                                    configs[lastSelectIndex].isEnable() && !forceChange) return lastSelectIndex;
                            }
                        }
        ...
        public int Select(List<Server> configs, int curIndex, string algorithm, FilterFunc filter,

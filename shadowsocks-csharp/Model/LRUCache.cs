...
namespace Shadowsocks.Model
{
    public class LRUCache<K, V>
    {
        ...
        public bool isTimeout(K key)
        {
            lock (_lock)
            {
                if ((DateTime.Now - _key_2_time[key]).TotalSeconds > _sweep_time) return true;
                return false;
            }
        }
        ...
        public V Get(K key)
        {
            lock (_lock)
            {
                if (_store.ContainsKey(key))
                {
                    ...
                    t = DateTime.Now;
                    while (_time_2_key.ContainsKey(t)) t = t.AddTicks(1);
                    _time_2_key[t] = key;
                    _key_2_time[key] = t;
                    return _store[key];
                }
        ...
        public void Sweep()
        {
            lock (_lock)
            {
                ...
                if (sweep > 0) Logging.Debug("sweep " + sweep + " items");
            }
        }
    }
}